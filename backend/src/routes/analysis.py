...
def analyze_with_openai(image_data, api_key, system_prompt, additional_context="", model="gpt-4o", filename="image.jpg"):
...
def analyze_with_gemini(image_data, api_key, system_prompt, additional_context="", model="gemini-2.0-flash", filename="image.jpg"):
...
def analyze_with_groq(image_data, api_key, system_prompt, additional_context="", model="openai/gpt-oss-120b", filename="image.jpg"):
...
def analyze_with_grok(image_data, api_key, system_prompt, additional_context="", model="grok-vision-beta", filename="image.jpg"):
...
def analyze_with_llama(image_data, api_key, system_prompt, additional_context="", model="llama-3.2-11b-vision-instruct", filename="image.jpg"):
...
def analyze_with_cohere(image_data, api_key, system_prompt, additional_context="", model="command-a-vision-07-2025", filename="image.jpg"):
...
def analyze_image():
    ...
    try:
        ...
        # Analyze with each selected service using global system prompt
        ...
        print(f"DEBUG: Global prompt length: {len(global_prompt)}")
        
        for service in selected_services:
            print(f"DEBUG: Processing service: {service}")
            if service == 'openai':
                result = analyze_with_openai(image_data, settings.openai_api_key, global_prompt, additional_context, settings.openai_model, filename)
                results.append(result)
            elif service == 'gemini':
                result = analyze_with_gemini(image_data, settings.gemini_api_key, global_prompt, additional_context, settings.gemini_model, filename)
                results.append(result)
            elif service == 'groq':
                result = analyze_with_groq(image_data, settings.groq_api_key, global_prompt, additional_context, settings.groq_model, filename)
                results.append(result)
            elif service == 'grok':
                result = analyze_with_grok(image_data, settings.grok_api_key, global_prompt, additional_context, settings.grok_model, filename)
                results.append(result)
            elif service == 'llama':
                result = analyze_with_llama(image_data, settings.llama_api_key, global_prompt, additional_context, settings.llama_model, filename)
                results.append(result)
            elif service == 'cohere':
                result = analyze_with_cohere(image_data, settings.cohere_api_key, global_prompt, additional_context, settings.cohere_model, filename)
                results.append(result)
            elif service == 'deepseek':
                result = analyze_with_deepseek(image_data, settings.deepseek_api_key, global_prompt, additional_context, settings.deepseek_model, filename)
                results.append(result)
            
            print(f"DEBUG: Result from {service}: success={result.get('success', False)}, title={result.get('title', '')[:50]}...")
